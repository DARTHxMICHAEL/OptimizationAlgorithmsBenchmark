...
- **Linear gradient - based (linear_gradient_max_search)** optimization performs first-order gradient ascent using a local linear approximation of the objective function. Gradients are estimated numerically via finite differences, which allows the method to operate without analytic derivatives but increases the number of function evaluations per iteration. The optimizer updates parameters directly along the estimated gradient direction with a fixed learning rate, making it simple and interpretable. As a purely local and deterministic method, it converges efficiently on smooth, unimodal surfaces but is highly sensitive to learning rate selection and finite-difference step size. The method is vulnerable to noise, non-smoothness, and local optima, and its performance degrades significantly on highly multimodal landscapes.

- **Analytical Hierarchy Process - based (ahp_max_search)** optimization adapts concepts from the Analytic Hierarchy Process to a continuous optimization setting by framing the problem as a discrete decision-making task. The method samples a finite set of candidate solutions uniformly from the search domain and constructs a pairwise comparison matrix based on relative objective values. Candidate priorities are derived from the principal eigenvector of this matrix, and the highest-ranked solution is selected as the optimum. This approach does not perform iterative improvement and therefore avoids local search dynamics, but its effectiveness depends strongly on sampling density and numerical stability of pairwise ratios. While computationally expensive due to quadratic pairwise comparisons, it serves as a decision-theoretic baseline rather than a competitive optimizer and is best suited for coarse global ranking rather than precise optimization.

- **Graph - based greedy (graph_greedy_max_search)** optimization models the continuous search space as a discretized grid graph, where nodes represent sampled points and edges connect local neighborhoods. Starting from a randomly chosen node, the algorithm performs greedy ascent by repeatedly moving to the neighboring node with the highest objective value, terminating when no improving move exists. This explicitly graph-constrained search introduces topological structure and resolution-dependent behavior, distinguishing it from continuous hill-climbing methods. The optimizer is deterministic after initialization and has bounded computational cost, but its accuracy is limited by grid resolution and it is prone to becoming trapped in local graph maxima. As a result, it provides a structured discrete baseline that highlights the impact of space discretization and neighborhood topology on optimization performance.
